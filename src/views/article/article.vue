...
const getCategoryName = (categoryNo) => {
  return categoryMap[categoryNo] || "其他";
};
//  狀態
...
const FilteredArticles = computed(() => {
  const filtered =
    ActiveCategory.value === "全部"
      ? articleList.value // 直接使用 ref.value
      : articleList.value.filter((a) => a.event === ActiveCategory.value);

  // 排序時使用我們轉換後的 `date` 欄位，而不是 API 原本的 `created_at`
...
//抓文章區API
const articleList = ref([]); //取代假資料 用來放抓來的所有文章
onMounted(async () => {
    ...
    if (Array.isArray(dataFromApi)) {
      articleList.value = dataFromApi.map((post) => {
        ...
        return {
          ...
          image: fullImageUrl, // <-- 使用修正後的完整 URL
          date: post.CREATED_AT,
          event: getCategoryName(post.CATEGORY_NO), // <-- 現在可以正常呼叫了
        };
      });
    }
  } catch (error) {
      ...
      >
        ...
        >
          ...
          <div class="article-text">
            ...
            <div class="read-more">
              <Button :onClick="readarticle" theme="info" size="sm"
                >閱讀更多</Button
              >
            </div>
          </div>
